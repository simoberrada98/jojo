...
function adviceFor(dbUrl: string): string[] {
  ...
  try {
    ...
    if (u.hostname.endsWith('.supabase.co')) {
      if (u.port === '6543')
        tips.push(
          'Use direct Postgres port 5432 (not the pooler 6543) for dumps'
        );
      if (!u.searchParams.has('sslmode'))
        tips.push('Append ?sslmode=require to the connection string for TLS');
    }
  } catch {}
...
async function main() {
  ...
  if (!args.forcePgDump && supabaseAvailable) {
    ...
    if (!ok) {
      console.warn(
        '[warn] Supabase CLI dump failed, trying pg_dump fallback...'
      );
    }
  }
  if (!ok) {
  ...
  if (!ok) {
    ...
    console.error(
      '- If using pg_dump, install PostgreSQL client tools and ensure pg_dump is in PATH.'
    );
    process.exit(1);
  }
