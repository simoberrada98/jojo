...
function optimizeTitle(product: ProductJSON): string {
  ...
  // Extract model name
  let modelMatch = originalTitle.match(/(Antminer|Whatsminer)\s+(S\d+\w*|L\d+|T\d+|M\d+|KA\d+|E\d+)/i)
  if (!modelMatch) {
  ...
  else if (originalTitle.match(/ethereum|eth|etc/i)) coinType = 'Ethereum Classic'
...
function optimizeHandle(title: string): string {
  ...
  return slugify(title)
    .replace(/\s+/g, '-')
    .substring(0, 100) // Keep handles reasonable length
}
...
function cleanDescription(bodyHtml: string): string {
  ...
  cleaned = cleaned.replace(/much cheaper than/gi, 'Competitive alternative to')
...
async function main() {
  ...
  for (const file of files) {
    ...
    console.log(`Meta: ${generateMetaDescription(optimized.title, optimized.body_html)}`)
    console.log('---')
