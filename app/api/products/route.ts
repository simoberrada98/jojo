...
import { transformToDisplayProduct, type Product } from "@/lib/types/product"
...
export async function GET(request: NextRequest) {
  ...
  try {
    logger?.api?.("GET", "/api/products", { id, handle, ids: idsParam, q, limit, offset })
    ...
    if (handle) {
      ...
      return jsonWithCache(transformToDisplayProduct(product as Product), DETAIL_CACHE_MAX_AGE_SECONDS)
    }
    ...
    if (idsParam) {
      ...
      const transformed = (products || []).map(p => transformToDisplayProduct(p as Product))
    ...
    const results = (products || []).map(p => transformToDisplayProduct(p as Product))
    const total = count || 0
    ...
    logger?.error?.("/api/products failure", error, { id, handle, ids: idsParam, q, limit, offset })
