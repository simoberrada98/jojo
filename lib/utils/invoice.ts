...
export const generateInvoicePDF = (data: InvoiceData) => {
  ...
  doc.text('MintyOS', 20, 20);
  
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('Premium Mining Hardware', 20, 28);
  doc.text('support@mintyos.com | +1 (555) 123-4567', 20, 34);
  ...
  doc.text(`$${data.subtotal.toLocaleString()} USD`, 190, summaryStartY, { align: 'right' });
  
  // Shipping
  doc.text('Shipping:', summaryX, summaryStartY + 7);
  doc.text(`$${data.shipping.toLocaleString()} USD`, 190, summaryStartY + 7, { align: 'right' });
  
  // Tax
  doc.text('Tax:', summaryX, summaryStartY + 14);
  doc.text(`$${data.tax.toFixed(2)} USD`, 190, summaryStartY + 14, { align: 'right' });
  ...
  doc.text(`$${data.total.toLocaleString()} USD`, 190, summaryStartY + 28, { align: 'right' });
  
  if (data.currency !== 'USD') {
    doc.setFontSize(9);
    doc.setTextColor(grayColor[0], grayColor[1], grayColor[2]);
    doc.text(`(${data.currency} equivalent shown at checkout)`, 190, summaryStartY + 35, { align: 'right' });
  }
  ...
  doc.text('For questions or support, contact us at support@mintyos.com', 105, footerY + 14, { align: 'center' });
  doc.text('MintyOS | Premium Mining Hardware | www.mintyos.com', 105, footerY + 20, { align: 'center' });
  
  // Save the PDF
  doc.save(`MintyOS_Invoice_${data.orderNumber}.pdf`);
};
